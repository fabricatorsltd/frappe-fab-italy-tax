...
from fab_italy_tax.vat_period_calculation import calculate_vat_period, is_split_payment
...
class TestVATPeriodCalculation(unittest.TestCase):
	...
	def test_split_payment_invoice_vat_stays_out_of_output_vat(self):
		document = build_vat_period(previous_credit_brought_forward=0.0)
				...
				return [
					{
						...
						"voucher_no": "FATT/2026/00035",
						"credit": 220.0,
						"debit": 0.0,
					},
					{
						"account": "VAT Split Payment - FAB",
						"voucher_type": "Sales Invoice",
						"voucher_no": "FATT/2026/00036",
						"credit": 110.0,
						"debit": 0.0,
					},
					{
						"account": "IVA 22% - FAB",
						"voucher_type": "Sales Invoice",
						"voucher_no": "FATT/2026/00037",
						"credit": 44.0,
						"debit": 0.0,
					},
				]
			if doctype == "Account":
				return ["IVA 22% - FAB", "VAT Split Payment - FAB"]
			if doctype == "Sales Invoice":
				return [
					{"name": "FATT/2026/00035", "vat_collectability": "S-Scissione dei Pagamenti"},
					{"name": "FATT/2026/00036", "vat_collectability": "S-Scissione dei Pagamenti"},
					{"name": "FATT/2026/00037", "vat_collectability": "I-Immediata"},
				]
			if doctype == "VAT Adjustment":
				return []
			raise AssertionError(f"Unexpected get_all call: {doctype}")
		...
		self.assertEqual(results["output_vat_total"], 44.0)
		self.assertEqual(results["final_payable_amount"], 44.0)

	def test_split_payment_credit_note_vat_stays_out_of_output_vat(self):
		document = build_vat_period(previous_credit_brought_forward=0.0)
		...
		def get_all_side_effect(doctype, **kwargs):
			...
			if doctype == "Sales Invoice":
				return [
					{"name": "FATT/2026/00040", "vat_collectability": "I-Immediata"},
					{"name": "NC/2026/00002", "vat_collectability": "S-Scissione dei Pagamenti"},
				]
			if doctype == "VAT Adjustment":
				return []
			raise AssertionError(f"Unexpected get_all call: {doctype}")
		...
		self.assertEqual(results["output_vat_total"], 220.0)

	def test_is_split_payment_reads_the_collectability_code(self):
		self.assertTrue(is_split_payment({"vat_collectability": "S-Scissione dei Pagamenti"}))
		self.assertFalse(is_split_payment({"vat_collectability": "I-Immediata"}))
		self.assertFalse(is_split_payment({"vat_collectability": "D-Differita"}))
		self.assertFalse(is_split_payment({"vat_collectability": ""}))
		self.assertFalse(is_split_payment({}))
